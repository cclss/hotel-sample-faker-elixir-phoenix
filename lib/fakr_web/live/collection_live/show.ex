    ...
    <Layouts.app flash={@flash} current_scope={@current_scope}>
      <div class="max-w-5xl mx-auto">
          ...
          <div>
            ...
            <div :if={any_published?(@collection.resources)} class="mt-3 space-y-2">
              <p class="text-sm text-gray-400">
                Public URL:
                <a href={~p"/@#{@username}/#{@collection.slug}"} class="text-cypress hover:underline" target="_blank">
                  /@{@username}/{@collection.slug}
                </a>
              </p>
              <div class="flex items-center gap-2">
                <a href={~p"/@#{@username}/#{@collection.slug}"} target="_blank" class="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs bg-cypress text-white rounded-lg hover:bg-cypress/90 transition font-medium">
        ...
        <div class="flex items-center justify-between mb-4">
          <h2 class="text-xl font-semibold text-peppercorn">Resources</h2>
          <button
            phx-click="open_resource_modal"
            class="inline-flex items-center px-3 py-2 text-sm bg-cypress text-white rounded-lg hover:bg-cypress/90 transition"
          >
        ...
        <div
          ...
          class="text-center py-12 bg-white rounded-xl border border-smoke"
        >
        ...
        <div class="space-y-4">
          <div
            ...
            class="bg-white rounded-xl border border-smoke p-6"
          >
            <div class="flex items-start justify-between">
              <div>
                <div class="flex items-center gap-2">
                  <h3 class="text-lg font-semibold text-peppercorn">{resource.name}</h3>
                  <span class={[
                ...
                <div
                  ...
                  class="mt-2 flex items-center gap-2 p-2 bg-cavendish/20 border border-cavendish rounded-lg"
                >
                  <.icon name="hero-exclamation-triangle" class="w-4 h-4 text-peppercorn shrink-0" />
                  <span class="text-xs text-peppercorn">
                ...
                <div :if={resource.published} class="mt-2 space-y-1">
                  <p class="text-xs font-mono bg-peppercorn text-green-400 px-3 py-1.5 rounded inline-block">
              ...
              <div class="flex items-center gap-2 flex-wrap justify-end">
                <button
                  ...
                  class="px-3 py-1.5 text-xs bg-cypress text-white rounded-lg hover:bg-cypress/90 transition font-medium"
                ...
                <button
                  ...
                  class={[
                    ...
                    if(resource.published_revision != resource.revision,
                      do: "bg-cavendish text-peppercorn hover:bg-cavendish/80 ring-2 ring-cavendish ring-offset-1",
                ...
                <.link
                  ...
                  class="text-sm text-cypress hover:underline"
                >
                  Edit
                </.link>
                <button
      ...
      <div
        :if={@show_resource_modal}
        class="fixed inset-0 z-50 flex items-center justify-center"
        phx-window-keydown="close_resource_modal"
        phx-key="Escape"
      >
        <div class="fixed inset-0 bg-black/50" phx-click="close_resource_modal"></div>
        <div class="relative bg-white rounded-xl shadow-xl p-6 w-full max-w-md mx-4 z-10">
          <div class="flex items-center justify-between mb-4">
            <h2 class="text-lg font-semibold text-peppercorn">New Resource</h2>
            <button phx-click="close_resource_modal" class="text-gray-400 hover:text-gray-600">
              <.icon name="hero-x-mark" class="w-5 h-5" />
            </button>
          </div>
          <.form for={@resource_form} id="new-resource-form" phx-submit="create_resource">
            <div class="space-y-4">
              <.input field={@resource_form[:name]} type="text" label="Resource Name" placeholder="e.g. users, products" required phx-mounted={JS.focus()} />
            ...
            <div class="mt-6 flex items-center gap-3">
              ...
              <button type="button" phx-click="close_resource_modal" class="text-sm text-gray-500 hover:underline">
                Cancel
              </button>
            </div>
          </.form>
